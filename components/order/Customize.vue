...
import { desserts } from '~/entities/Extras'
</script>
...
<template>
    ...
    <Card>
        ...
        <CardContent class="grid gap-6">
            <RadioGroup default-value="1" class="grid grid-cols-3 gap-4">
                <div v-for="item in desserts" :key="item.id">
                    <RadioGroupItem :id="`dessert-${item.id}`" :value="item.id.toString()" class="peer sr-only" />
                    <Label :for="`dessert-${item.id}`"
                        class="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" class="mb-3 h-6 w-6">
                            <rect width="20" height="14" x="2" y="5" rx="2" />
